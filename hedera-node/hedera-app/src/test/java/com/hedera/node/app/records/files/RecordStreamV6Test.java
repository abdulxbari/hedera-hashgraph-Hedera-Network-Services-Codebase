...
public class RecordStreamV6Test {
    ...
    public void serializeTest(final List<SingleTransactionRecord> singleTransactionRecords,final boolean compress) throws Exception {
        ...
        try (final RecordFileWriter recordFileWriterV6 = new RecordFileWriter(recordFilePath, RecordFileFormatV6.INSTANCE, compress)) {
            ...
            try (final SidecarFileWriter sidecarFileWriter = new SidecarFileWriter(sidecarPath ,compress , 256)) {
                ...
                for (SingleTransactionRecord singleTransactionRecord : singleTransactionRecords) {
                    // serialize
                    final SerializedSingleTransactionRecord item = RecordFileFormatV6.INSTANCE.serialize(singleTransactionRecord, BLOCK_NUM, VERSION);
                    ...
                    final RecordStreamItem parsedRecordStreamItem = RecordStreamItem.PROTOBUF.parse(
                            BufferedData.wrap(item.protobufSerializedRecordStreamItem().toByteArray()));
                    assertEquals(new RecordStreamItem(singleTransactionRecord.transaction(), singleTransactionRecord.record()), parsedRecordStreamItem);
                    // check sidecar items
                    assertEquals(singleTransactionRecord.transactionSidecarRecords().size(), item.sideCarItems().size());
                    assertEquals(singleTransactionRecord.transactionSidecarRecords().size(), item.sideCarItemsBytes().size());
                    for (int i=0; i < singleTransactionRecord.transactionSidecarRecords().size(); i++) {
                        final TransactionSidecarRecord sideCarRecord = singleTransactionRecord.transactionSidecarRecords().get(i);
                        final TransactionSidecarRecord sideCarRecord2 = item.sideCarItems().get(i);
                        assertEquals(sideCarRecord, sideCarRecord2);
                        final Bytes sideCarRecordBytes = item.sideCarItemsBytes().get(i);
                        assertEquals(TransactionSidecarRecord.PROTOBUF.toBytes(sideCarRecord).toHex(),sideCarRecordBytes.toHex());
                        // write sidecar item
                        sidecarFileWriter.writeTransactionSidecarRecord(sideCarRecord.sidecarRecords().kind(), sideCarRecordBytes);
                    }
                }
                // close sidecar file
                ...
                final Bytes endRunningHash = RecordFileFormatV6.INSTANCE.computeNewRunningHash(STARTING_RUNNING_HASH_OBJ.hash(), serializedSingleTransactionRecords);
                // check running hash
                messageDigest.reset();
                ...
                // write footer
                recordFileWriterV6.writeFooter(endRunningHashObj, BLOCK_NUM, List.of(sidecarMetadata));
                // close record file
                ...
                assertEquals(HEX.formatHex(previousHash), recordFileWriterV6.endObjectRunningHash().hash().toHex());
                assertEquals(HEX.formatHex(fileHash), recordFileWriterV6.uncompressedFileHash().toHex());
                ...
                for(int i=0; i < singleTransactionRecords.size(); i++) {
                    ...
                    final RecordStreamItem recordStreamItem = readRecordStreamFile.recordStreamItems().get(i);
...
//        final var consensusTimeInstant = Instant.ofEpochSecond(consensusTime.getSeconds(), consensusTime.getNanos());
...
//        assertEquals(HEX.formatHex(recordStreamObjectBytes), HEX.formatHex(item.hashSerializedRecordStreamItem().toByteArray()));
...
////        var expectedRunningHash = cryptography.calcRunningHash(runningHashStartHash, recordObjectHash, DigestType.SHA_384);
