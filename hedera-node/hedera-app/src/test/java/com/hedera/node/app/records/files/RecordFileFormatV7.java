...
public final class RecordFileFormatV7 implements RecordFileFormat {
    ...
    public SerializedSingleTransactionRecord serialize(
        ...
        try {
            ...
            final List<Bytes> sideCarItemsBytes = singleTransactionRecord.transactionSidecarRecords().stream().map(TransactionSidecarRecord.PROTOBUF::toBytes).toList();
            // now hash sidecar items
            Bytes sidecarHash = null;
            if (!sideCarItemsBytes.isEmpty()) {
                final MessageDigest sidecarMessageDigest = MessageDigest.getInstance(DigestType.SHA_384.algorithmName());
