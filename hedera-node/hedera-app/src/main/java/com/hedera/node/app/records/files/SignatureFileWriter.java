...
public final class SignatureFileWriter {
     ...
     */
    public static void writeSignatureFile(@NonNull final Path recordFilePath,
        ...
        try (final var fileOut = Files.newOutputStream(sigFilePath, StandardOpenOption.CREATE_NEW)) {
            ...
            if (writeMetadataSignature) {
                ...
                HashingOutputStream hashingOutputStream = new HashingOutputStream(
            ...
            log.error("closeCurrentAndSign ::  :: Fail to generate signature file for {}",
                    recordFilePath,
                    e);
    ...
    static Path getSigFilePath(@NonNull final Path recordFilePath) {
        ...
        if (recordFileName.endsWith(COMPRESSION_ALGORITHM_EXTENSION)) {
            recordFileName = recordFileName.substring(0, recordFileName.length() - COMPRESSION_ALGORITHM_EXTENSION.length());
        }
