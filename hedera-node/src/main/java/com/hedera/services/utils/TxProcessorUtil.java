...
public class TxProcessorUtil {
    ...
    public static Wei chargeForEth(
            BigInteger userOfferedGasPrice,
            Wei gasCost,
            long maxGasAllowanceInTinybars,
            MutableAccount mutableSender,
            MutableAccount mutableRelayer,
            Wei allowanceCharged,
            long gasPrice,
            long gasLimit,
            long value) {
    ...
    public static long gasPriceTinyBarsGiven(
            LivePricesSource livePricesSource,
            final Instant consensusTime,
            boolean isEthTxn,
            HederaFunctionality txType) {
        return livePricesSource.currentGasPrice(
                consensusTime, isEthTxn ? HederaFunctionality.EthereumTransaction : txType);
    }

    public static long calculateGasUsedByTX(
    ...
    public static long calculateGasUsedByTX(
            final long txGasLimit, PrecompileMessage message, final int maxRefundPercent) {
        long usedGas = txGasLimit - message.getGasRemaining();
        return Math.max(usedGas, txGasLimit - txGasLimit * maxRefundPercent / 100);
    }
    ...
    public static MutableAccount getMutableSender(
            HederaWorldState.Updater updater, Account sender) {
        final var senderAccount = updater.getOrCreateSenderAccount(sender.getId().asEvmAddress());
        return senderAccount.getMutable();
    }

    public static MutableAccount getMutableRelayer(
            HederaWorldState.Updater updater, Account relayer) {
    ...
    public static long calculateRefundEth(
            long gasLimit,
            long gasPrice,
            MutableAccount mutableRelayer,
            Wei allowanceCharged,
    ...
    public static void sendFeesToCoinbase(
        ...
        MutableAccount mutableCoinbase =
                updater.getOrCreate(
    ...
    public static TransactionProcessingResult buildTransactionResult(
            boolean isCompletedSuccess,
            List<Log> logs,
            long sbhRefund,
            long gasUsedByTransaction,
            long gasPrice,
            Address mirrorReceiver,
            Bytes output,
