...
public class TreasuryReturnHelper {
    ...
    EntityNumPair finishNft(
        ...
        final var rootNftId = rootKey.asNftNumPair().nftId();
        if (burn) {
            final var burnedNft = nfts.get(rootNftId);
            nextKey = burnedNft.getNext();
            nfts.remove(rootNftId);
        } else {
            final var returnedNft = nfts.getForModify(rootNftId);
