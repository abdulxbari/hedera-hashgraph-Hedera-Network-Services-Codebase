...
class ScheduleDeleteHandlerTest extends ScheduleHandlerTestBase {
    ...
    void scheduleDeleteHappyPath() throws DecoderException, PreCheckException {
        realPreContext = new PreHandleContextImpl(mockStoreFactory, scheduleDeleteTransaction(), testConfig);
    ...
    void scheduleDeleteFailsIfScheduleMissing() throws PreCheckException {
        final var schedule = scheduleDeleteTransaction();
        realPreContext = new PreHandleContextImpl(mockStoreFactory, schedule, testConfig);
        BDDMockito.given(schedulesById.get(testScheduleID.scheduleNum())).willReturn(null);

        Assertions.assertThrowsPreCheck(() ->
                subject.preHandle(realPreContext), ResponseCodeEnum.INVALID_SCHEDULE_ID);
    }
    ...
    void scheduleDeleteScheduleIsImmutable() throws PreCheckException {
        ...
        BDDMockito.given(scheduleInState.adminKey()).willReturn(Optional.empty());
