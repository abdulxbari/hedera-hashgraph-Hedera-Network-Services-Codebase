...
public class SelectorDescriptions {
    ...
    static @NonNull String retrieveSignatureEndpoint(long sig) {
                            ...
                            () -> {
                                try (var stream = in) {
                                    ...
                                    if (DEBUGGING_HTTP_REQUEST)
                                        System.out.printf(
                                                "SignatureListingJsonBodyHandler: have '%s'%n",
                                                new String(bytes));
    ...
    static MethodSignatureInfo identifySelector(
        ...
        Function<Object, MethodSignatureInfo> wrapError =
                (@NonNull Object msg) -> {
                    Exception ex =
                            msg instanceof Exception e ? e : new RuntimeException(msg.toString());
                    errorAccumulator.put(selector, ex);
                    return null;
                };

        var request = requestTemplate.uri(URI.create(retrieveSignatureEndpoint(selector))).build();

        if (DEBUGGING_HTTP_REQUEST)
            System.out.printf("identifySelector: request: '%s'%n", request.toString());
        ...
        if (response.statusCode() != HTTP_OK)
            return wrapError.apply(
                    "web service returned status other than HTTP_OK: %d"
                            .formatted(response.statusCode()));

        ;
        var signatures = response.body().get();
        if (signatures.count != 1) return wrapError.apply("selector unknown");

        var signature = signatures.results.get(0);
        var name = signature.textSignature.split("[(]", 2)[0];
        return new MethodSignatureInfo(selector, name, signature.textSignature);
    }

    public record SelectorIdentificationResults(
            @NonNull Map<Long, MethodSignatureInfo> identifications,
            @NonNull Map<Long, Exception> errors) {
    ...
    public @NonNull SelectorIdentificationResults identifySelectors(
            @NonNull Collection<Long> selectors) {
        final var om =
        ...
        final var client =
                HttpClient.newBuilder()
                        ...
                        .build();
        final var requestTemplate =
                HttpRequest.newBuilder()
                        .GET()
                        .setHeader("Accept", "application/json")
                        .timeout(REQUEST_TIMEOUT);
        var errorAccumulator = new HashMap<Long, Exception>();
        var result =
                selectors.stream()
                        .flatMap(
                                selector ->
