...
public class VTableEntryRecognizer extends CodeRecognizer {
    ...
    public VTableEntryRecognizer(Assembly assembly) {
        super(assembly);
    }
    ...
    public enum State {
        ...
        START(null, "DUP1", "PUSH1", "PUSH2" /*, "JUMPDEST" */) {
            @Override
            public Optional<State> acceptCodeLine(
        ...
        Optional<State> next() {
            if (null == nextState)
                throw new IllegalStateException(
                        "attempting to go to default next state where it must be computed, %s"
                                .formatted(this));
        ...
        State(Next nextState, @NonNull String... expectingMnemonic) {
            if (expectingMnemonic.length == 0)
                throw new IllegalArgumentException(
                        "State must have at least one mnemonic expected");
    ...
    @NonNull List<MacroLine> macros = new ArrayList<>();

    @NonNull List<String> trace = new ArrayList<>();
    ...
    void gotSelectorNodeMatch() {
        trace(
                "gotSelectorNodeMatch: startByteOffset %04X nextByteOffset %04X selector %08X jumpOffset %04X"
    ...
    void gotVTableStart() {
        trace(
                "gotVTableStart: startByteOffset %04X nextByteOffset %04X"
                        .formatted(startByteOffset, nextByteOffset));

        final var macro =
                new VTSelectorTableStart(startByteOffset, nextByteOffset - startByteOffset);
    ...
    void gotVTableFailJump() {
        ...
        trace(
                "gotVTableFailJump: startByteOffset %04X nextByteOffset %04X failOffset %04X"
                        .formatted(startByteOffset, nextByteOffset, failOffset));

        final var macro =
                new VTSelectorLeafNodeEnd(
                        startByteOffset, nextByteOffset - startByteOffset, failOffset);
    ...
    @Override
    public @NotNull CodeRecognizer.Results end() {
        var vtLeafNodes =
                macros.stream()
        ...
        var replacements =
                Stream.of(
                                ...
                                Stream.of(
                                        new LabelLine(
                                                failOffset,
                                                VTSelectorLeafNodeEnd.FAIL_DESTINATION,
