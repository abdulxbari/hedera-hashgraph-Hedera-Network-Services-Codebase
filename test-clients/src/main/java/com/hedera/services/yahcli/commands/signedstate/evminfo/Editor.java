...
import com.hedera.services.yahcli.commands.signedstate.evminfo.Editor.Edit.Kind;
...
public class Editor {
    ...
    @NonNull private final List<Line> baseLines;
    @NonNull private List<Edit> edits;
    ...
    protected record Edit(
        ...
        public Edit(
            ...
            this(
                    editor.validateLineRange(lineRange),
                    newLines,
                    editor.validateByteRange(lineRange, byteRange));
        }
        ...
        public Edit(
            ...
            this(
                    ...
                    lineRange,
                    newLines,
                    editor.getByteRangeFromLines(editor.validateLineRange(lineRange)));
        }
        ...
        public String toString() {
            return "Edit{lineRange=%s, byteRange=%s, newLines=%s}"
                    .formatted(lineRange, byteRange, newLines);
        }
    }
    ...
    public @NonNull List<Line> merge() {
        ...
        final Consumer<Edit> inserter =
                edit -> merged.addAll(edit.lineRange().from(), edit.newLines());
        final Consumer<Edit> deleter =
                edit -> merged.subList(edit.lineRange().from(), edit.lineRange().to()).clear();
     ...
     * list
     */
    protected static void arrangeEditsForMerge(@NonNull List<Edit> edits) {
    ...
    @SuppressWarnings(
            "java:S2293") // Replace type specification...with diamond operator (for `Range<...>`)
    protected @NonNull Range<Byte> getByteRangeFromLines(@NonNull final Range<Line> lineRange) {
        ...
        if (null == firstLast[0])
            firstLast = getCodeRangeFromLines(new Range<Line>(0, lineRange.to()));
        if (null == firstLast[0]) return Range.<Byte>empty();
        return new Range<Byte>(
                firstLast[0].getCodeOffset(),
                firstLast[1].getCodeOffset()
                        + firstLast[1].getCodeSize() /*exclusive at this end*/);
    }
    ...
    protected @NonNull Range<Line> getLineRangeFromCode(@NonNull final Code codeLine) {
        ...
        if (firstLine < 0)
            throw new IndexOutOfBoundsException(
                    "can't find code line at code offset 0x%04X".formatted(atOffset));
        ...
        if (lastLine < 0)
            throw new IndexOutOfBoundsException(
                    "can't find code line with last offset at 0x%04X, code line %s"
                            .formatted(toOffset, codeLine));
    ...
    protected @NonNull Range<Line> validateLineRange(@NonNull final Range<Line> lineRange) {
        if (!isValidLineRange(lineRange))
            throw new IllegalArgumentException("EDIT: invalid lineRange");
        return lineRange;
    }
     ...
     */
    protected boolean isValidByteRange(
            @NonNull final Range<Line> lineRange, @NonNull final Range<Byte> byteRange) {

        final var actualBaseLinesByteRange = getByteRangeFromLines(lineRange);
        if (byteRange.from() < actualBaseLinesByteRange.from()
                || byteRange.from() > actualBaseLinesByteRange.to()) return false;
        return byteRange.to() >= byteRange.from()
                && byteRange.to() <= actualBaseLinesByteRange.to();
    }
    ...
    protected static boolean hasNoOverlappingEdits(@NonNull final List<Edit> edits) {
        ...
        final var emptyAndProperRanges =
                edits.stream()
                        .map(Edit::lineRange)
                        .collect(Collectors.partitioningBy(Range<Line>::isEmpty));
        ...
        var kinds =
                edits.stream()
                        ...
                        .collect(
                                groupingBy(
                                        k -> k,
                                        () -> new TreeMap<Kind, Long>(comparing(Kind::name)),
                                        Collectors.counting()));
        for (final var k : Kind.class.getEnumConstants()) kinds.putIfAbsent(k, 0L);

        var basicInfo =
                "%d baselines, %d edits %s"
                        .formatted(baseLines.size(), edits.size(), kinds.toString());
