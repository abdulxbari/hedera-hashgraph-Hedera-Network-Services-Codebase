...
public class ERCDirectTokenCallSuite extends HapiApiSuite {
                                                            ...
                                                            asHexedSolidityAddress(
                                                                    spec.registry()
                                                                            .getAccountID(
                                                                                    RECIPIENT)),
                                                            100)
                                                    .via("TRANSFER_WITH_INSUFFICIENT_BALANCE")
                                                    .payingWith(ACCOUNT)
                                                    .hasKnownStatus(CONTRACT_REVERT_EXECUTED),
                                            contractCallWithFunctionAbi(
                                                            tokenAddress,
                                                            ...
                                                            asHexedSolidityAddress(
                                                                    spec.registry()
                                                                            .getAccountID(
                                                                                    RECIPIENT)),
                                                            tokenTransferAmount)
                                                    .via("TOKEN_NOT_ASSOCIATED_TO_ACCOUNT")
                                                    .payingWith(ACCOUNT)
                                                    .hasKnownStatus(CONTRACT_REVERT_EXECUTED),
                                            contractCallWithFunctionAbi(
                                                            tokenAddress,
                                                            ...
                                                            100)
                                                    .via(
                                                            "TRANSFER_SENDER_WITH_INSUFFICIENT_BALANCE")
                                                    .payingWith(EMPTY_BALANCE_ACCOUNT)
                                                    .hasKnownStatus(CONTRACT_REVERT_EXECUTED));
                                }))
                .then(
                        getAccountBalance(ACCOUNT)
                                .payingWith(ACCOUNT)
                                .hasTokenBalance(FUNGIBLE_TOKEN, tokenTransferAmount),
                        childRecordsCheck(
                                "TRANSFER_ZERO_AMOUNT",
                                CONTRACT_REVERT_EXECUTED,
                                recordWith().status(INVALID_ACCOUNT_AMOUNTS)),
                        childRecordsCheck(
                                "TRANSFER_WITH_INSUFFICIENT_BALANCE",
                                CONTRACT_REVERT_EXECUTED,
        ...
        return defaultHapiSpec("erc20DirectTokenCallsWithEOA")
                .given(
                        cryptoCreate(spender)
                                .exposingCreatedIdTo(
                                        id -> spenderMirrorAddr.set(asHexedSolidityAddress(id))),
                        newKeyNamed(ecdsaKey).shape(SECP256K1_ON),
                        cryptoTransfer(
                                        tinyBarsFromToWithAlias(
                                                DEFAULT_PAYER, ecdsaKey, ONE_HUNDRED_HBARS))
                                .via(creation),
    ...
    private HapiApiSpec directTokenCallsPerformance() {
        ...
        return defaultHapiSpec("directTokenCallsPerformanceTest")
                ...
                .then(
                        finishThroughputObs("contractCallWithFunctionAbi")
                                .gatedByQuery(
                                        () ->
